...
# الجمعية الأولى - 20 سهم × 20,000 جنيه = 400,000 جنيه
GAMEIA_1 = {
    "name": "الجمعية الأولى",
    "amount": 400000,
    "data": {
...
# الجمعية الثانية - 25 سهم × 25,000 جنيه = 625,000 جنيه
GAMEIA_2 = {
    "name": "الجمعية الثانية",
    "amount": 625000,
    "data": {
...
def get_gameia1_this_month():
    key = get_current_month_key()
    return GAMEIA_1["data"].get(key, None)
...
def build_monthly_message():
    ...
    msg = f"📅 *تذكير الجمعية - {month_name} {now.year}*\n"
    msg += "━━━━━━━━━━━━━━━━━━\n\n"
    ...
    msg += "\n━━━━━━━━━━━━━━━━━━\n\n"
    msg += "💰 *الجمعية الثانية* - الأسهم حسب الترتيب\n"
    msg += "_(راجع الجدول بالأمر /gameia2)_\n"
...
async def gameia1_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg += f"المبلغ: 400,000 جنيه شهرياً\n"
    msg += "━━━━━━━━━━━━━━━━━━\n"
    for month_key, info in GAMEIA_1["data"].items():
        marker = "◀️ " if month_key == current_key else ""
        note = f" ({info['note']})" if info['note'] else ""
        msg += f"{marker}`{month_key}` — {info['name']}{note}\n"
    await update.message.reply_text(msg, parse_mode="Markdown")

async def gameia2_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = "💰 *جدول الجمعية الثانية*\n"
    msg += f"المبلغ: 625,000 جنيه للسهم\n"
    msg += "━━━━━━━━━━━━━━━━━━\n"
    for num, info in GAMEIA_2["data"].items():
        note = f" ({info['note']})" if info['note'] else ""
        msg += f"السهم {num}: *{info['name']}*{note}\n"
    await update.message.reply_text(msg, parse_mode="Markdown")
...
def main():
    ...
    # تذكير تلقائي كل أول الشهر الساعة 9 الصبح
    job_queue = app.job_queue
    job_queue.run_monthly(monthly_reminder, when=datetime.now().replace(day=1, hour=9, minute=0, second=0).time(), day=1)
